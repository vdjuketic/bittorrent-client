def decode_bencode(bencoded_value):
    return decode_bencode_with_end(bencoded_value)[0]


def decode_bencode_with_end(bencoded_value):
...
def decode_list(bencoded_list):
    ...
    while chr(bencoded_list[current_char]) != "e":
        decoded_part, chars_read = decode_bencode_with_end(bencoded_list[current_char:])
...
def decode_dictionary(bencoded_dictionary):
    ...
    while chr(bencoded_dictionary[current_char]) != "e":
        decoded_key, key_chars_read = decode_bencode_with_end(
            bencoded_dictionary[current_char:]
        )
        current_char += key_chars_read

        decoded_value, value_chars_read = decode_bencode_with_end(
            bencoded_dictionary[current_char:]
        )
